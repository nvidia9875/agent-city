...
- 介入カード（公式警報一斉配信 / 多言語アラート / ルート誘導 / デマ監視 / 要支援者支援 / 支援優先度リバランス / 誤配分是正トリアージ など）の効果を比較しやすい設計とする。
...
- シミュレーションは安定化 / 危機拡大 / タイムアップで終了し、結果モーダルを表示（上限60tick、安定化/危機拡大は12tick連続で判定）
...
- 避難中住民は開放中の避難所（SHELTER/SCHOOL）へ向かい、収容可能なら `SHELTERED` に遷移する
- 避難所の収容率に応じて `OPEN/CROWDED` が動的に変化し、満員時は待機住民のストレスが上昇する
...
- **LeftTimeline**: LG画面では上端位置を維持したまま下端まで伸び、右カラムに Canvas と介入パネルを縦配置
- **BottomInterventions**: 介入カード（災害シナリオごとに内容が変化、横スクロール＋矢印で操作）
- **BottomInterventions**: 介入ポイントは全体プールで管理し、使用時はコスト連動（目安33%）のポイントをクールダウン中に段階回復
- **中央Canvas**: 3D街 + 住民 + クリック/ホバー用ツールチップ
- **起動フロー**: 初回は3ページのミッションガイド（カードスライド）を表示し、最後まで読むと設定モーダルへ遷移
- **起動モーダル**: サイズ/人数/建物/地形/災害シナリオを設定し、スタートで開始
- **起動モーダル**: 住民の気分（温かめ/中立/冷ため）と年齢層（若年/バランス/高齢）も選択可能
- **起動モーダル**: 公式情報遅延、曖昧さ、デマ強度、多言語対応率、検証速度の初期パラメータを設定可能
- **起動モーダル**: 介入ポイント（介入カードに使える総量）を設定可能
- **結果モーダル**: 終了時にメトリクス推移と分布を可視化し、仮想時間の現実換算（仮想1分=現実1時間）を表示
- **離脱ガード**: ゲーム進行中は更新/タブクローズでブラウザ確認を表示し、戻る操作はアラート表示のうえページ遷移を抑止する
...
- 避難所の混雑・満員は避難遅延を生み、現場のストレスと混乱の再上昇要因になる
...
- AI 行動決定では吹き出し専用セリフ（bubbleLine）を生成し、イベント文とは分離して表示できる
- 吹き出しは個体ごとに0〜5秒の開始ずれを持ち、10秒単位で表示/非表示が切り替わる
- 同じ住民の吹き出しが連続して表示されないよう、5秒表示ごとに一度オフになる
- 吹き出し表示は全住民の約8%を抽選し、同時表示は最大5件まで（会話の見えやすさを優先）
...
- 空白クリックで吹き出し解除
- 左上の拡大 / 縮小ボタンでカメラをズーム可能
- スタート / 再スタート時のシミュレーション速度は常に1xにリセットされる
...
- AI 行動決定は `message`（タイムライン要約）と `bubbleLine`（住民の口語セリフ）を分離し、`bubbleLine` は感情・個性タグ・プロフィールを反映して生成する
- 吹き出しは `buildAgentBubble` でルール生成を常時実行し、エージェント状態（気分/ストレス/役割/脆弱性/近傍会話/災害シナリオ）を合成して作る
- `SIM_FORCE_AI_BUBBLE_TEXT=true` の場合のみ、吹き出しLLM（既定: `gemini-2.5-flash`）で上書きを試行する
- 吹き出しLLMは意思決定LLMと分離し、`SIM_AI_BUBBLE_*` で別レート制御・429バックオフを行う
- 429（RESOURCE_EXHAUSTED）時は吹き出しLLMを止めても、ルール生成の吹き出しは継続して表示する
- SIM_END 時に Vector Search から類似イベント/噂汚染度の集計を取得し、結果画面に表示する
- `VERTEX_VECTOR_DEPLOYED_INDEX_ID` 未設定時は endpoint の `deployedIndexes` から自動解決を試行する
- Vector Insights が `ready` 以外のときは理由（reason）を結果画面に表示する
- Vector Insights が `ready` の場合も、Embedding待機・近傍0件・ID照合失敗などの診断情報を表示する
- Vector Insights のクラスタ件数は「近傍件数」ではなく「対象メモリ件数」を表示し、0件誤解を避ける
- Vector Insights は `TALK/RUMOR/OFFICIAL/ALERT/CHECKIN` を中心に会話スレッドを構築し、終盤の発話ラインを可視化する
- 結果画面の Vector Insights は「どんな会話があったか / 不安ポイント / 解決の進展」を主表示にし、技術診断は折りたたみ表示にする
- 会話スレッドでは「介入後に噂優勢から公式優勢へ反転したtick」を検出し、結果画面でハイライト表示する
- 会話スレッドは通常2発話以上を優先しつつ、Embedding待機/近傍不足時は直近60tickのタイムラインから単発発話も補完して0件表示を回避する
- `TALK` は話者文と返答文を分けた会話形式で生成し、タイムラインには「A→B」の会話ログとして記録する
- `TALK` アクションの吹き出しは会話文を直接表示し、相手側にも返答セリフを即時反映する
- AIエージェントの会話相手選定は近傍AIを優先し、AI同士の対話が見えやすいようにする
- 住民吹き出しは可視中でも最新テキストへ更新し、会話内容の差し替え遅延を防ぐ
- クライアントのWS接続は既定で `ws://<host>:3001` を使用し、`NEXT_PUBLIC_USE_MOCK_WS=true` の場合のみ mockWs を利用する
- Embeddingクールダウンが発生した集計では、汚染系指標（噂の汚染度/最大汚染度/鎮静化率）を `N/A` として表示する
- `rumorOverlap.score` は「公式近似率」ではなく、近傍内の非公式割合を汚染度として算出する
- メモリには `simulation_id` を付与し、Vector Insights 集計は同じ `simulation_id` の記録だけを対象にする
...
- 介入カードにはコストとクールダウンを設定し、介入ポイント内での最適化を楽しめる
- 介入ポイントは 5秒ごとに +10pt 回復し、回復ゲージと次回回復までの秒数をUI表示する
- 介入カードは効果カテゴリごとのアイコン付きで表示し、直感的に選べるUIとする
- 介入カードの連携コンボ（例: デマ監視→公式警報）で追加効果が発動し、介入順序の戦略性を高める
- コンボ誘導は専用ナビゲータを使わず、介入カード自体を色分け（起点=寒色/完成=暖色）した強めのアニメ枠で強調表示する
- 介入カードの使用回数は1ゲームあたり10回上限で、残り回数をUIに表示する
- 誤配分是正トリアージは「不要出動の停止」と「誤情報由来要請の訂正」を同時に行い、誤配分率を短時間で下げる切り札として機能する
